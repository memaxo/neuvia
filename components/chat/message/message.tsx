...
import { CheckIcon, XIcon, FileIcon, Pencil as PencilEditIcon, Sparkles as SparklesIcon, File as DocumentIcon } from 'lucide-react';
...
const PurePreviewMessage = ({
  ...
  const verificationStatus = message.metadata?.verificationMetadata?.verificationStatus as 'pending' | 'in_progress' | 'completed' | 'failed' | undefined;
  ...
  const summaryVersion = useMemo(() => {
    if (!summaryVersionId || !chatContext?.verification?.summaryVersions) return null;
  ...
  const handleSectionCorrection = useCallback((sectionTitle: string) => {
  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        className={cn(
          "group/message mx-auto w-full max-w-3xl px-4",
          {
        ...
        <div
          className={cn(
            'flex w-full gap-4 group-data-[role=user]/message:ml-auto group-data-[role=user]/message:max-w-2xl',
            {
              ...
              'group-data-[role=user]/message:w-fit': mode !== 'edit' && !correctionMode,
            },
          )}
        >
          ...
          <div className="flex w-full flex-col gap-2">
            ...
            {(isSummaryMessage || isVerificationMessage) && verificationStatus && (
              ...
              <div className={cn(
                ...
                {!isReadonly && verificationStatus !== 'completed' && (
                  <VerificationActions 
                    ...
                    onHistory={chatContext?.verification?.summaryVersions?.length > 1 ? handleViewHistory : undefined}
                  />
                )}
              </div>
            )}
            ...
            {message.content && !isSummaryMessage && mode === 'view' && !correctionMode && (
                        ...
                        {toolName === 'search' ? (
                          <div className="rounded border p-3 text-sm">
                            ...
                            {result.data && Array.isArray(result.data) ? (
                              <ul className="space-y-2">
                                {result.data.map((item: any, idx: number) => (
                                  <li className="border-t pt-2" key={idx}>
                                    <a className="font-medium text-blue-600 dark:text-blue-400" href={item.url} rel="noopener noreferrer" target="_blank">
                                      {item.title}
                                    </a>
                              ...
                              <div className="text-gray-500">No search results available</div>
                            )}
                          </div>
                        ) : toolName === 'extract' || toolName === 'scrape' ? (
                          <div className="rounded border p-3 text-sm">
                            <h4 className="mb-2 font-medium">{toolName === 'extract' ? 'Extracted Content' : 'Scraped Content'}</h4>
                            ...
                            {args?.url && (
                              <div className="mt-2 text-xs">
                                ...
                                <a className="text-blue-600 dark:text-blue-400" href={args.url} rel="noopener noreferrer" target="_blank">
                                  {args.url}
                                </a>
                              </div>
                            )}
                      ...
                      {toolName === 'getDocument' ? (
                        <div className="animate-pulse rounded border p-3">
                          ...
                          <p className="text-sm text-gray-500">
                            {args && 'query' in args ? `Query: ${args.query}` : 'Retrieving document...'}
                          </p>
                        </div>
                      ) : null}
                    </div>
