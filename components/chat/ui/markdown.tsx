...
const HighlightedChanges = ({ 
  ...
  return (
    <span className={cn('rounded px-1 py-0.5', bgColor)}>
      {text}
    </span>
  );
};
...
const components: Partial<Components> = {
  ...
  a: ({ node, children, href, ...props }) => {
    ...
    if (href?.startsWith('#highlight:')) {
      const type = href.replace('#highlight:', '') as 'added' | 'removed' | 'changed';
      return (
        <HighlightedChanges text={children as string} type={type} />
      );
    }
  ...
  td: ({ node, children, ...props }) => {
    return <td className="whitespace-normal px-4 py-3" {...props}>{children}</td>;
  },
};
...
const NonMemoizedMarkdown = ({ 
  ...
  if (isSummary && !disableCollapsible && !content.includes('#collapse:') && content.length > 500) {
