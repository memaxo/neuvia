...
export function ChatInterface({ initialMode, patientId }: ChatInterfaceProps) {
  ...
  const handleVerificationComplete = useCallback((verificationItems: VerificationItem[]) => {
  ...
  const handleGenerateReport = useCallback((notes: string) => {
      ...
      progressInterval = setInterval(() => {
        setProcessProgress(prev => {
          ...
          if (processingMsg && processingMsg.id) {
            updateProgressMessage(processingMsg.id, newProgress, processingPhase(newProgress));
          }
...
`;
      ...
      toast({
        ...
        description: "Failed to process document. Please try again or contact support.",
        variant: "destructive"
      });
  ...
  }, [workflow, patientId, toast, addSystemMessage, updateProgressMessage, startVerification, processingPhase]);
      ...
      <div className="bg-muted/50 flex items-center gap-2 border-b px-4 py-2">
        ...
        {isProcessing && (
          <>
            ...
            <span className="text-muted-foreground text-xs">{processPhase}</span>
          </>
        )}
      </div>
    );
  ...
  const handleMessageSubmit = async (message: string) => {
    ...
    if (inVerificationMode && (lowerMessage === 'confirm' || lowerMessage === 'approve')) {
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="border-b p-4">
        <DocumentUploader 
          disabled={isProcessing || (workflow.workflowStep !== "idle" && workflow.workflowStep !== "complete")}
      ...
      {workflow.workflowStep !== "idle" && workflow.workflowStep !== "complete" && !inVerificationMode && (
        <div className="px-4 pt-4">
