...
import { Loader2, Search, FileText, Stethoscope, AlertTriangle } from 'lucide-react';
  ...
  return (
    <Card className={`w-full ${className}`}>
      <CardHeader>
        ...
        <CardDescription>
          Generate differential diagnoses with confidence ratings based on patient data
        </CardDescription>
      </CardHeader>
      ...
      <Tabs onValueChange={setActiveTab} value={activeTab}>
        <TabsList className="mx-6">
          ...
          <TabsTrigger disabled={!result} value="conditions">Conditions</TabsTrigger>
        ...
        <CardContent className="pt-6">
          <TabsContent value="diagnosis">
            <div className="space-y-4">
              <div className="flex flex-col space-y-2">
                <label className="text-sm font-medium" htmlFor="diagnosis-query">
                ...
                <p className="text-muted-foreground text-xs">
                  Leave empty for a standard differential diagnosis based on the patient data. Add specific symptoms, concerns, or questions to focus the diagnosis.
                </p>
              </div>
            ...
            {result && (
              <div className="space-y-4">
                ...
                <div className="text-muted-foreground flex justify-between text-sm">
                  <span>
                    {result.conditions.length} {result.conditions.length === 1 ? 'condition' : 'conditions'} identified
                  </span>
                  {result.timing && (
                    <span>
          ...
          <TabsContent value="conditions">
            {result && (
              <div className="space-y-4">
                <h3 className="font-medium">Differential Diagnosis ({result.conditions.length})</h3>
                
                <div className="space-y-4">
                  {result.conditions.map((condition, index) => (
                    <div 
                      className={`rounded-md border p-4 ${
                        condition.confidence > 0.7 ? 'border-l-4 border-l-red-500' : 
                        condition.confidence > 0.4 ? 'border-l-4 border-l-amber-500' : 
                    ...
                    >
                      <div className="mb-2 flex items-center justify-between">
                        ...
                        <div 
                          className={`rounded px-2 py-1 text-sm ${
                            condition.confidence > 0.7 ? 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200' : 
                            condition.confidence > 0.4 ? 'bg-amber-100 text-amber-800 dark:bg-amber-900 dark:text-amber-200' : 
                      ...
                      <div className="mb-3">
                        <h5 className="text-muted-foreground mb-1 text-xs uppercase">Rationale</h5>
                        <p className="text-sm">{condition.rationale}</p>
                      </div>
                      
                      {condition.treatments && condition.treatments.length > 0 && (
                        <div>
                          <h5 className="text-muted-foreground mb-1 text-xs uppercase">Possible Treatments</h5>
