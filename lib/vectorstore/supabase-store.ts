...
import { DocumentMetadataSchema, type DocumentMetadata, type DocumentSearchResult } from '@/lib/schemas/document-types';
...
export class EnhancedSupabaseVectorStore extends SupabaseVectorStore {
  ...
  async addDocument(document: Document): Promise<string> {
    // Validate metadata
    const validatedMetadata = DocumentMetadataSchema.partial().parse(document.metadata || {});
      ...
      chunks.push(
        new Document({
          ...
          metadata: {
            ...
            chunk_total: Math.ceil(pageContent.length / (chunkSize - chunkOverlap)),
            is_chunk: true,
          },
        })
      );
  ...
  async deleteDocuments(filter: Record<string, any>): Promise<number> {
    try {
      ...
      let selectQuery = this.supabaseClient
        .from('document_chunks')
        .select('id');
      ...
      for (const [key, value] of Object.entries(filter)) {
        if (key.startsWith('metadata.')) {
          ...
          selectQuery = selectQuery.filter(`metadata->>${metadataKey}`, 'eq', value);
        } else {
          // For regular columns
...
export function createSupabaseVectorStore(): EnhancedSupabaseVectorStore {
  ...
  // Create vector store
  return new EnhancedSupabaseVectorStore(
    embeddings,
    supabaseClient,
    {
