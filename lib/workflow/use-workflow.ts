...
type WorkflowStateRow =
  Database['public']['Tables']['workflow_states']['Row'] & {
...
export function useWorkflow(options: UseWorkflowOptions = {}) {
  ...
  const createNewWorkflowState = useCallback(async () => {
    ...
    const currentStep = isDbWorkflowStep(state.step) ? state.step : 'idle'
  ...
  const initiateVerification = useCallback(
    async (extractedDocument: any, messageId?: string) => {
      if (!userId || !chatId) {
    ...
    async (
      ...
      try {
        ...
        if (
          ...
          throw new Error(
            `Invalid workflow step for correction: ${currentStep}`
          )
        }
        ...
        const currentVerificationMetadata =
          workflowState.verification_metadata || {}
        ...
        setVerificationMetadata(
          updatedVerificationMetadata as VerificationMetadata
        )
  ...
  const beginReportGeneration = useCallback(
    async (reportMetadata?: Record<string, any>) => {
      if (!userId) {
