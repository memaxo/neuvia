...
export function useReport(
  documentInput: ResearchDocument | VerifiedDocument | null
) {
  ...
  const [reportDocument, setReportDocument] = useState<ReportDocument | null>(
    null
  )
  ...
  const formatReport = useCallback(
    async (format: ReportFormat = reportFormat) => {
      if (!reportData) {
        setStatus({
          status: 'error',
          progress: 0,
