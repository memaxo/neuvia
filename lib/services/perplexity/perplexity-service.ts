...
const ResearchResultSchema = z.object({
  text: z.string().describe("The full research text with all details and findings"),
  sources: z.array(z.object({
    title: z.string().optional().describe("The title of the source"),
    url: z.string().describe("The URL of the source"),
    snippet: z.string().optional().describe("A brief excerpt or summary of the source content"),
  })).describe("The list of sources used in the research"),
  summary: z.string().describe("A concise summary of the research findings"),
  keyFindings: z.array(z.string()).describe("List of key findings or takeaways from the research")
});

// More detailed schema for medical diagnosis results
const MedicalDiagnosisSchema = z.object({
  text: z.string().describe("The full medical diagnosis report with all details"),
  sources: z.array(z.object({
    title: z.string().optional().describe("The title of the source"),
    url: z.string().describe("The URL of the source"),
    snippet: z.string().optional().describe("A brief excerpt or summary of the source content"),
  ...
  keyFindings: z.array(z.string()).describe("Key clinical findings from the analysis"),
  patientSummary: z.string().optional().describe("Overview of critical patient information"),
  differentialDiagnosis: z.array(z.object({
    condition: z.string().describe("Name of the condition or disorder"),
    confidence: z.number().min(0).max(100).describe("Confidence rating as a percentage"),
    rationale: z.string().describe("Detailed explanation supporting this diagnosis"),
    treatments: z.string().describe("Recommended treatments and follow-up actions")
  })).optional().describe("Detailed breakdown of potential diagnoses with confidence ratings"),
  recommendations: z.array(z.string()).optional().describe("Recommended diagnostic tests, assessments, or specialist referrals")
});

// Create output parsers from the schemas
const outputParser = StructuredOutputParser.fromZodSchema(ResearchResultSchema);
const medicalDiagnosisParser = StructuredOutputParser.fromZodSchema(MedicalDiagnosisSchema);
...
export class PerplexityService {
  ...
  private extractDifferentialDiagnoses(text: string): string[] {
    ...
    const conditionRegex = /(?:##?\s*|[\*\d]+\.\s*)(?:Condition|Disorder|Diagnosis)[^\n]*?:\s*([^\n]+)/gi;
    let conditionMatch;
    ...
    const confidenceRegex = /(?:##?\s*|[\*\d]+\.\s*)Confidence[^\n]*?:\s*(\d+)%/gi;
    ...
    if (diagnoses.length > 0 && confidences.length === diagnoses.length) {
      return diagnoses.map((diagnosis, index) => `${diagnosis} (${confidences[index]} confidence)`);
    }
    ...
    if (diagnoses.length === 0) {
      ...
      while ((sectionMatch = sectionRegex.exec(text)) !== null) {
        ...
        if (sectionTitle.includes('Diagnos') || 
   ...
   */
  private handleParsingError(error: unknown, text: string): ParsedResearchOutput {
    ...
    console.warn(`[PerplexityService] Failed to parse text (preview): ${textPreview}`);
    
    // Determine if this is likely a medical diagnosis text
    const isMedicalDiagnosis = text.includes('Differential Diagnosis') || 
    ...
    return {
      ...
      keyFindings: isMedicalDiagnosis ? this.extractDifferentialDiagnoses(text) : this.extractKeyFindings(text),
  ...
  private async processApiResponse(
    ...
    try {
      ...
      if (isMedicalDiagnosis) {
        ...
        if (medicalResult.differentialDiagnosis && medicalResult.differentialDiagnosis.length > 0) {
          keyFindings = medicalResult.differentialDiagnosis.map(diagnosis => 
            `${diagnosis.condition} (${diagnosis.confidence}% confidence)`
          );
          
          // Add recommendations to key findings if available
          if (medicalResult.recommendations && medicalResult.recommendations.length > 0) {
      ...
      sources = Array.isArray(completion.sources) && completion.sources.length > 0
        ? completion.sources
            .filter(Boolean)
    ...
    return {
      ...
      keyFindings: parsedOutput.keyFindings?.length ? parsedOutput.keyFindings : this.extractKeyFindings(text)
    };
  }
  ...
  async performDeepResearch(
    ...
    try {
      ...
      const parser = options?.isMedicalDiagnosis ? medicalDiagnosisParser : outputParser;
      ...
      const completion = await generateText({
        model: perplexity(options?.model || researchConfig.providers.perplexity.model), // Use model from config
        prompt: promptText,
        temperature: options?.temperature || this.getConfigValueForDepth('temperature', options?.depth),
        maxTokens: options?.maxTokens || this.getConfigValueForDepth('maxTokens', options?.depth),
        system: systemPrompt,
  ...
  async performMedicalDiagnosis(
    query: string,
    patientData: string,
    options?: Omit<EnhancedResearchOptions, 'isMedicalDiagnosis' | 'patientData'>
  ): Promise<ResearchResult> {
    ...
    const enhancedOptions = {
      ...
      maxTokens: options?.maxTokens || this.getConfigValueForDepth('maxTokens', 'comprehensive'),
      temperature: options?.temperature || this.getConfigValueForDepth('temperature', 'comprehensive')
    };
    
    // Use a specialized query format for medical diagnosis that works with our new prompt structure
    const diagnosticQuery = "Provide a comprehensive differential diagnosis based on the patient data";
  ...
  private extractKeyFindings(text: string): string[] {
    ...
    const findingsSection = text.match(/(?:##?\s*(?:Key\s*)?Findings|Observations|Results)\s*\n+([^#]+)/i);
    if (findingsSection && findingsSection[1]) {
      const sectionPoints = findingsSection[1].split(/\n+/).map(p => p.trim()).filter(p => p.length > 0);
    ...
    return runWithWorkflow(
      ...
      async () => {
        ...
        const researchParser = StructuredOutputParser.fromZodSchema(
          z.object({
            text: z.string().describe("The full research text with all details"),
            sources: z.array(z.object({
