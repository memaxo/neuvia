...
export async function extractPatientSummary(
  ...
  try {
    ...
    const extractionSequence = createExtractionSequence(workflowId, options, onProgress);
...
export async function processCorrection(
  ...
  try {
    ...
    const correctionSequence = createCorrectionSequence(workflowId, options, onProgress);
    ...
    const result = await runWithWorkflow<string>(
      ...
      {
        onProgress: (progress) => onProgress?.(progress, 'Processing correction'),
        workflowId
      }
    );
