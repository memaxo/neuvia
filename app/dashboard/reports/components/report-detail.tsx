...
export function ReportDetail({ report }: ReportDetailProps) {
  ...
  const documentReferences: DocumentReference[] = (typeof report.source_documents === 'object' && report.source_documents !== null)
    ...
    <>
      <div className="bg-card space-y-6 rounded-lg p-6">
        ...
        <div className="space-y-4">
          {/* Clinical Content */}
          <div>
            <h3 className="text-foreground text-lg font-medium">Clinical Content</h3>
            <div className="prose prose-invert" dangerouslySetInnerHTML={{ __html: report.content || '' }} />
          </div>
          
          {/* Document References */}
          <div>
            <h3 className="text-foreground text-lg font-medium">Document References</h3>
            {documentReferences && documentReferences.length > 0 ? (
              <ul className="list-disc space-y-1 pl-5">
                {documentReferences.map((reference: DocumentReference, idx: number) => (
                  <li className="text-foreground text-sm" key={idx}>
                    <span className="font-semibold">{reference.citationId || `Doc-${idx+1}`}: </span>
                    {reference.title}
                    {reference.page && <span> - Page {reference.page}</span>}
                    <span className="text-muted-foreground ml-2 text-xs">({reference.type})</span>
                  </li>
                ))}
              </ul>
            ) : (
              <p className="text-muted-foreground text-sm">No document references available.</p>
            )}
          </div>

          {/* Differential Diagnoses */}
          <div>
            <h3 className="text-foreground text-lg font-medium">Differential Diagnoses</h3>
            {differentialDiagnoses.length > 0 ? (
              ...
              <p className="text-muted-foreground text-sm">No differential diagnoses available.</p>
            )}
          </div>
          {/* Model Metadata */}
          <div>
            <h3 className="text-foreground text-lg font-medium">Model Metadata</h3>
            <p className="text-muted-foreground text-sm">Version: {modelMetadata.version}</p>
            <p className="text-muted-foreground text-sm">Training Date: {modelMetadata.training_date}</p>
            {modelMetadata.reasoning_chain && modelMetadata.reasoning_chain.length > 0 && (
              <div className="mt-2">
          ...
          {/* Validation Metadata */}
          <div>
            <h3 className="text-foreground text-lg font-medium">Validation Metadata</h3>
            <p className="text-muted-foreground text-sm">Checksum: {validationMetadata.checksum}</p>
          ...
          {/* Compliance Metadata */}
          <div>
            <h3 className="text-foreground text-lg font-medium">Compliance Metadata</h3>
            <p className="text-muted-foreground text-sm">Data Retention Policy: {complianceMetadata.data_retention_policy}</p>
            {complianceMetadata.regulatory_flags && complianceMetadata.regulatory_flags.length > 0 && (
              <div className="text-muted-foreground mt-2 text-sm">
