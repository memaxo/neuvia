...
export async function POST(req: NextRequest) {
  try {
        ...
        if (!patientData) {
          ...
          if (typeof document === 'object') {
            if ('content_text' in document && document.content_text) {
              ...
              patientData = typeof document.content === 'string' 
                ? document.content 
    ...
    const queryText = query || (researchOptions.isMedicalDiagnosis 
    ...
    try {
      ...
      const result = researchOptions.isMedicalDiagnosis && researchOptions.patientData
        ? await perplexityService.performMedicalDiagnosis(queryText, researchOptions.patientData, researchOptions)
        : await perplexityService.performDeepResearch(queryText, researchOptions);
